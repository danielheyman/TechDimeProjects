...
var ajaxChat = {
	...
	socketUpdate: function(data) {
		...
		if(xmlDoc) {
			...
			if(false && (this.showChannelMessages || xmlDoc.firstChild.getAttribute('mode') != '1') && !this.channelSwitch) {
	...
	getUserNodeString: function(userID, userName, userRole) {
		if(this.userNodeString && userID == this.userID) {
			...
			var str	= '<div id="'
					...
					+ '">'
                    + '<span style="border-radius:5px; -moz-border-radius:5px; -o-border-radius:5px; -webkit-border-radius:5px; display:inline-block; width:30px; height:30px;background:url(http://surfsavant.com/image.php?id=' + userID + ')"> </span><span style="top:-10px; left:5px; position:relative;">'
					+ newName
					+ '</span></a>'
	...
	getUserNodeStringItems: function(encodedUserName, userID, isInline) {
		...
		if(encodedUserName != this.encodedUserName) {
			menu 	= '<li><a target="_blank" href="http://www.surfsavant.com/directory/i-'
                    + userID
                    + '">Open directory</a></li>'
					...
					+ '</a></li>'
                    + '<li><a href="javascript:ajaxChat.insertText(\'@'
					+ encodedUserName
					+ ' \');">Mention</a></li>'
			...
			if(this.userRole == 2 || this.userRole == 3) {
				menu	+= '<li><a target="_blank" href="http://www.surfsavant.com/admin-users/'
                        + userID
                        + '">Manage User</a></li>'
			...
			if(this.userRole == 1 || this.userRole == 2 || this.userRole == 3) {
				menu	+= '';
                        //+= '<li><a href="javascript:ajaxChat.sendMessageWrapper(\'/join\');">'
	...
	addMessageToChatList: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
        ...
        var span = document.getElementsByClassName("spanProfile");
        if(span[0])
        {
            for(var x = 0; x < span.length; x++)
            {
                if(span[x].innerHTML != 'ChatBot')
                {
                    span.style.backgroundImage="url('img_tree.png')";
                    var div = document.getElementById(span[x].getAttribute("parent"));
                    div.innerHTML = '<img src="' + span[x].getAttribute("src") + '">' + div.innerHTML;
                }
            }
        }
	},
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		var colon;
        var mentions = (userNameOriginal != "ChatBot") ? '<a class="mentionbox" title="Private Message" onclick="ajaxChat.insertMessageWrapper(\'/msg ' + userNameOriginal + ' \');">P</a>' + '<a class="mentionbox" title="Mention User" onclick="ajaxChat.insertText(\'@' + userNameOriginal + ' \');">M</a>' : '';
        //var mentions = (userNameOriginal != "ChatBot") ? '<a class="options" title="Private Message" onclick="ajaxChat.insertText(\'/msg ' + userNameOriginal + ' \');">Private Msg</a>' + '<a class="options options2" title="Mention" onclick="ajaxChat.insertText(\'@' + userNameOriginal + ' \');">Mention</a>' : '';
        var mentionClass = "";
        //var mentionClass = (userNameOriginal != "ChatBot") ? 'optionsClass' : '';
		...
		var dateTime = this.settings['dateFormat'] ? '<span class="dateTime">'
						+ this.formatDate('(%H:%i)', dateObject) + '</span> ' : '';
        
        var message = this.replaceText(messageText);
        message = message.split(" ");
        for(var x = 0; x < message.length; x++)
        {
            if(message[x][0] == "@" && message[x].length > 2)
            {
                message[x] =  '<span class="green">' + message[x] + '</span>'; 
            }
        }
        
        var directory = (userName == 'ChatBot') ? '' : ' target="_blank" href="http://www.surfsavant.com/directory/i-' + userID + '"';
        
        message = message.join(" ");
        
		return	'<div style="padding-left:40px;" id="'
				...
				+ ' dir="'
				+ this.baseDirection
				+ '"><a style="text-decoration:none;" ' + directory + '>'
                + '<span style="border-radius:5px; -moz-border-radius:5px; -o-border-radius:5px; -webkit-border-radius:5px; margin-left:-35px; position:absolute; display:inline-block; width:30px; height:30px;background:url(http://surfsavant.com/image.php?id=' + userID + ')"> </span>'
				+ userName
				+ '</a></span>'
				//+ dateTime
				//+ colon
                ...
                //+ mentions 
                + '<span style="display:inline-block;"> </span><span class="message">'
				+ message
				+ '</span></div>';
	},
